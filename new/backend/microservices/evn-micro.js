...
module.exports = {
    cityspecific: async function (city) {
        ...
        var callback = new Promise((res, rej) => {
            event.find({ evnCity: city },{evnPincode: 1, evnDescription: 1, evnName: 1, evnDate1: 1, evnDate2: 1, evnInterests: 1, Image:1, evnLocation: 1, evnCity: 1, evnTargetAge: 1}, async function (err, OBJ) {
                if (err) {
                    console.log(err)
                }
                else {
    ...
    all: async function () {
        var callback = new Promise((res, rej) => {
            event.find({}, {evnPincode: 1, evnDescription: 1, evnName: 1, evnDate1: 1, evnDate2: 1, evnInterests: 1, Image:1, evnLocation: 1, evnCity: 1, evnTargetAge: 1},async function (err, OBJ) {
                if (err) {
                    console.log(err)
                }
                else {
